...
namespace EK.Datos.Kontrol.MSSQL
{
        ...
        : DAOBaseGeneric<m.Kontrol.Interfaces.IPersonalizarCampo>, d.Kontrol.Interfaces.IPersonalizarCampos
    {
        ...
        public async Task<int> GetConfigCountValue(Dictionary<string, object> parametros)
        {
            try
            {
                return await helper.GetResultAsync(USP_PERSONALIZAR_CAMPOS_VALORES_SELECT, CommandType.StoredProcedure, parametros);
            }
            catch (Exception ex)
            {
                throw ex;
