...
namespace EK.Common.Utils
{
        ...
        : DynamicObject, ITableEntity
    {
        ...
        public ElasticEntity(Dictionary<string, object> properties)
            : this()
        {
            foreach (var prop in properties) {
                if (prop.Key == "PartitionKey")
                {
                    this.PartitionKey = prop.Value.ToString();
                }
                else if (prop.Key == "RowKey")
                {
                    this.RowKey = prop.Value.ToString();
                }
                else
                {
        ...
        public IDictionary<string, object> GetValues() {
            ...
            retValue.Add("PartitionKey", this.PartitionKey);
            retValue.Add("RowKey", this.RowKey);
            retValue.Add("Timestamp", this.Timestamp);
