...
namespace EK.Datos.SCV.MSSQL
{
    public class Tickets
      : d.Kontrol.DAOBaseGeneric<m.SCV.Interfaces.ITicket>, d.SCV.Interfaces.ITickets
    { 
        private const string USP_SPV_Tickets_SELECT = "usp_spv_tickets_select";
        private const string USP_SPV_Incidencias_CONSULTAS = "usp_spv_Incidencias_select";
        private const string USP_SPV_ReIncidencias_CONSULTAS = "usp_spv_ReIncidencias_select";
        ...
        public Tickets(m.Kontrol.Interfaces.IContainerFactory factory, d.Kontrol.Interfaces.IDBHelper helper)
            : base(factory, helper, USP_SPV_Tickets_SELECT, null, "spv_ticket")
        { }

        public async Task<List<m.SCV.Interfaces.ITicket>> getGridDashBoard(Dictionary<string, object> parametros)
        {
            try
            {
                return await helper.CreateEntitiesAsync<m.SCV.Interfaces.ITicket>(USP_SPV_Tickets_SELECT, CommandType.StoredProcedure, parametros);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public async Task<List<m.SCV.Interfaces.IReporteFallaIndicador>> getUsersDashBoard(Dictionary<string, object> parametros)
        {
            try
            {
                return await helper.CreateEntitiesAsync<m.SCV.Interfaces.IReporteFallaIndicador>(USP_SPV_Tickets_SELECT, CommandType.StoredProcedure, parametros);
            }
            catch (Exception ex)
            {
                throw ex;
