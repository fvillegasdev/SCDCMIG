namespace EK.Modules.SCP.Pages.RegistroPublicoPropiedad {
    "use strict";
    const config: page.IPageConfig = global.createPageConfig("RegistroPublicoPropiedad", "kontrol");
    let ml: any = config.getML();
    
    export class Edicion extends page.Base {
        saveForm(props: page.IProps, item: EditForm): any {
            let model: any = item
                .addID()
                .addClave()
                .addNombre()
                .addEstatus()
                .addDate("FechaInscripcion")
                .addObject("Ciudad")
                .addString("Partida")
                .addString("Folio")
                .addString("Libro")
                .addString("Seccion")
                .addVersion()
                .toObject();
            return model;
        };
        render(): JSX.Element {
            return <page.Main {...config} pageMode={PageMode.Edicion} onSave={this.saveForm}>
                <View />
                <Edit />
            </page.Main>;
        };
    };
    class Edit extends page.Base {
        render(): JSX.Element {
            return <page.Edit>
                <Column size={[12, 12, 12, 12]} style={{ paddingTop: 8 }}>
                    <page.OptionSection
                        id={config.id}
                        subTitle="Registro Publico de Propiedad"
                        icon="fa fa-pencil-square-o" collapsed={false} hideCollapseButton={true}>
                        <Row>
                            <input.Clave size={[12, 12, 2, 2]} maxLength={50} validations={[validations.required()]} />
                            <input.Nombre size={[12, 12, 8, 8]} validations={[validations.required()]} />
                            <checkBox.Status size={[12, 12, 2, 2]} />
                            <input.Integer id="Partida" size={[12, 4, 4, 4]} />
                            <input.Integer id="Folio" size={[12, 4, 4, 4]} />
                            <input.Integer id="Libro" size={[12, 4, 4, 4]} />
                            <input.Integer id="Seccion" size={[12, 4, 4, 4]} />
                            <select.Estados id="Ciudad" size={[12, 4, 4, 4]}/>
                            <input.Date id="FechaInscripcion" size={[12, 4, 4, 4]}/>
                        </Row>
                    </page.OptionSection>
                </Column>
            </page.Edit>;
        };
    };
    class View extends page.Base {
        render(): JSX.Element {
            return <page.View>
                <Column size={[12, 12, 12, 12]}>
                    <page.OptionSection
                        id={config.id}
                        subTitle="Registro Publico de Propiedad"
                        icon="fa fa-info-circle" collapsed={false} hideCollapseButton={true}>
                        <Row>
                            <label.Clave size={[12, 12, 2, 2]} />
                            <label.Nombre size={[12, 12, 8, 8]} />
                            <label.Estatus id="Estatus" size={[12, 12, 2, 2]} />

                            <label.Label id="Partida" size={[12, 4, 4, 4]} />
                            <label.Label id="Folio" size={[12, 4, 4, 4]} />
                            <label.Label id="Libro" size={[12, 4, 4, 4]} />
                            <label.Label id="Seccion" size={[12, 4, 4, 4]} />
                            <label.Entidad id="Ciudad" size={[12, 4, 4, 4]} />
                            <label.Label id="FechaInscripcion" size={[12, 4, 4, 4]} />

                        </Row>
                    </page.OptionSection>
                </Column>
            </page.View>;
        };
    };
};


