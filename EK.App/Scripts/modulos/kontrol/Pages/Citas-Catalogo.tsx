...
namespace EK.Modules.Kontrol.Pages.Citas {
    "use strict";
    const config: page.IPageConfig = global.createPageConfig("citas", "kontrol", ["expedientes", "ScvClientes"]);
    let PAGE_ID = "Citas";
        ...
        onEntityLoaded(props: page.IProps): any {
            ...
            if (entidad) {
                if (entidad.IdExpedienteREF) {
                    ...
                    //
                    Forms.updateFormElement(config.id, "fromREFC", true);
                };
            };
        };
        componentWillReceiveProps(nextProps: IEdicion, nextState: IEdicion): any {
            ...
            if (global.hasChanged(this.props.cliente, nextProps.cliente)) {
                if (global.isSuccessful(nextProps.cliente)) {
                    Forms.updateFormElement(config.id, "Cliente", global.getData(nextProps.cliente));
                    //
            ...
            return <page.Edit>
                <Column size={[12, 12, 12, 12]}>  
                    <page.OptionSection
                        id={PAGE_ID}
                        subTitle={PAGE_ID}
                        icon="far fa-calendar-alt" collapsed={false} hideCollapseButton={true}>
                        <Row>
                            ...
                            {Forms.getValue("fromREFE", config.id) === true || Forms.getValue("fromREFC", config.id) === true
...
//<input type="text"  data-format="hh:mm A" className="form-control clockface_1 clockface-open"/>  hh:mm A
    ...
    const MapView: any = global.connect(class extends React.Component<IMapViewProps, IMapViewState> {
        ...
        static props: any = (state: any) => {
            return {
                ...
                asentamiento: Forms.getValue("Localidad", config.id, state),
        ...
        componentWillReceiveProps(nextProps: IMapViewProps): void {
            if (global.hasChanged(this.props.asentamiento, nextProps.asentamiento)) {
                Forms.updateFormElement(config.id, "Geolocalizacion", null);
