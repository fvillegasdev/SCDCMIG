namespace EK.Modules.SCV.Pages.postventa.RUBA.AgendaContratista {
    const PAGE_ID: string = "AgendaContratista";
    ...
    export const resetAgenda: () => void = (): void => {
        ...
        /*let p: any = global.assign({
            ...
            idExpediente: EK.Store.getState().global.reporte$reprogramaciones[0].IdExpediente,
            estatusAgenda: "REP",
    ...
    const listHeaderOrdenTrabajo: JSX.Element =
        <Column size={[12, 12, 12, 12]} className="fixheaderlist">
            <Row>
                ...
                <Column size={[6, 6, 6, 6]} className="list-default-header">{"Cliente"}</Column>
    ...
    interface IAgendaContratistaProps extends page.IProps {
        ...
        saveAgendaRep?: (item: any) => void;
        changeContratista?: (item: any) => void;
        tipoAgenda?: any;
    };

    export const AgendaContratista: any = global.connect(class extends React.Component<IAgendaContratistaProps, IAgendaContratistaProps> {
        ...
        static dispatchs: any = (dispatch: Redux.Dispatch<any>) => ({
            saveAgenda: (item: any): void => {
                ...
                //setTimeout(() => {
                //    let x = EK.Global.confirmTrueFalse("¿Desea Eliminar la planificacion anterior?", "Eliminar Planificación anterior", () => {
            ...
            changeContratista: (item: any): void => {
                global.dispatchAsyncPut("load::CHANGECONTRATISTAOT", "base/kontrol/AgendaSPV/Get/SaveAgendaChangeContratista", item);
            }
        });
            ...
            if (hasChanged(nextProps.agendaActualizada, this.props.agendaActualizada) && global.isSuccessful(nextProps.agendaActualizada)) {
                ...
                switch (item.Estado) {
                    case 5: // eliminado con exito
                        ...
                        if (idPagina === 'DashBoardAgendaContratista') {
        ...
        onSelectBotonEstatus(Estatus?: any): void {
            ...
            if (Estatus === "ATE") {
                ...
                EK.Global.confirm("Presione Confirmar ", "Atender Cita", () => {
                    ...
                    //var patt = new RegExp(/^[A-Za-z0-9\s\.\,\ñ\ÑÀ-ÿ\!\?\'\"\:\(\)\/\¿\¡\;]+$/g);
                    //if (item.OrdenTrabajo.AgendaDetalle.Observaciones !== undefined && item.OrdenTrabajo.AgendaDetalle.Observaciones !== null && item.OrdenTrabajo.AgendaDetalle.Observaciones.trim() !== '') {
                    //    var res = patt.test(item.OrdenTrabajo.AgendaDetalle.Observaciones);
                    //    if (!res) {
                    //        global.warning('El texto ingresado contiene caracteres no validos', 'Atencion');
                    //        return;
                    //    }
            ...
            if (Estatus === "REP") {
                ...
                EK.Global.confirm("Presione Confirmar para Reprogramar la Planificación", "Reprogramar Planificación", () => {
                    ...
                    let foundOT = allOTs.filter(x => x.ID === item.IdOrdenTrabajo)[0];
                    item.OrdenTrabajo.IdCat = foundOT.IdCat;
                    console.log(item)
                    //item.OrdenTrabajo.AgendaDetalle = global.assign(item.OrdenTrabajo.AgendaDetalle, {
                    //    IdMotivo: motivoProceso.ID,
                    //    Observaciones: observaciones,
        ...
        onUpdateAgendaClick() {
            ...
            let allOTs = EK.Store.getState().forms.ReportesFallas.form.OrdenesTrabajo.initialValue;
            let IdCat = allOTs.filter(x => x.ID === entidades[0].IdOrdenTrabajo)[0] !== undefined ? allOTs.filter(x => x.ID === entidades[0].IdOrdenTrabajo)[0].IdCat : null;
           //console.log(allOTs)
            //console.log(IdCat)
                                                ...
                                                return <Row style={{ margin: 0 }}>
                                                    ...
                                                    <Column size={[2, 2, 2, 2]} className="listItem-center-header">
                                                        <span className="badge badge-success bold">
                                                            <a onClick={(e) => { window.open(["#/scv/pv/ReportesFallas/", item.IdFolio].join(""), '_blank') }} style={{ color: "white" }}>{item.IdFolio}</a>
                                                        </span>&nbsp;/&nbsp;
                                                        <span className="badge badge-info bold">
                                                            <a rel="noopener noreferrer" href="javascript:void(0)" style={{ color: "white" }} onClick={(value) => { agendaRUBA.onOrdenTrabajoClick(item.IdOrdenTrabajo) }}>{item.IdOrdenTrabajo}</a>
                                                        </span>
                                                    </Column>
                                                ...
                                                <List
                                                    ...
                                                    listHeader={<Column size={[12, 12, 12, 12]} className="fixheaderlist">
                                                        <Row>
                                                            ...
                                                            <Column size={[3, 3, 3, 3]} className="list-default-header bold">{"Ubicación"}</Column>
                                                    ...
                                                    formatter={(index: number, item: any) => {
                                                        return <Row style={{ margin: 0 }}>
                                                            ...
                                                            <Column size={[9, 9, 9, 9]} className="listItem-default-header listItem-overflow">{!(item.Cliente) ? null : <span><span className="badge badge-success bold">{item.Cliente.ID}</span>&nbsp;{item.Cliente.Nombre}</span>}</Column>
                                                            <Column size={[3, 3, 3, 3]} className="listItem-default-header listItem-overflow">{!(item.Ubicacion) ? null : <span className="badge badge-info bold">{item.Ubicacion.ClaveFormato}</span>}</Column>
                                                ...
                                                <div className="tab-pane" id="tab_agenda_calendar">
                                                    <Row>
                                                        <Column style={{ marginLeft: "7px", marginTop: "-27px", marginBottom: "39px" }}>
                                                            <Button size={[4, 4, 4, 4]} className="btn btn-md btn-editing" style={{ backgroundColor: '#8ad48c', color: "#FFFFFF", width: "250px", position: "absolute", right: "40px" }} icon="fas fa-sync-alt" onClick={this.onUpdateAgendaClick.bind(this)}> Actualizar Calendario 2 </Button>
                                                        </Column>
                                                    </Row>
                        ...
                        <Row>
                            <page.SectionList
                                ...
                                mapFormToEntity={(form: EditForm, entidades?: any): any => {
                                    ...
                                    let _item: EditForm = Forms.getForm('AgendaNew');
                                    
                                    retValue.FechaFin = new Date(fechaFin.getTime() - (fechaFin.getTimezoneOffset() * 60000)).toISOString();
                                    var nuevaFInicio = new Date(retValue.FechaInicio);
                                    //console.log(nuevaFInicio, _item['FechaInicio']);
                                    retValue.FechaInicio = nuevaFInicio;
                                    console.log(retValue)
                                    if (this.equalDates(nuevaFInicio, _item['FechaInicio'], "=") ) {
                                        global.warning("La fecha y hora de la reserva no puede ser igual a la de la OT");
                                        return null;
                                    }

                                    //nuevaFInicio.setHours(nuevaFInicio.getHours() - 6);
                                    //retValue.FechaInicio = nuevaFInicio.toISOString();
                                    //retValue.FechaInicio.setHours(retValue.FechaInicio.getHours() - 2);
                                    //console.log(retValue )
                                    return retValue;
                                }}
                                formatter={(index: number, item: any) => {
...
import agendaContratistaPV = EK.Modules.SCV.Pages.postventa.RUBA.AgendaContratista;