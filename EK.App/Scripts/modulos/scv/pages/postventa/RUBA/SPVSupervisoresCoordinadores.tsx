namespace EK.Modules.SCV.Pages.postventa.RUBA.SPVSupervisoresCoordinadores {
    const PAGE_ID: string = "SPVSupervisoresCoordinadores";
    ...
    const REPORTE_SUPERVISORES_ASIGNADOS_ID: string = PAGE_ID + "$supervicoresAsignados";
    ...
    const config: page.IPageConfig = global.createPageConfig(PAGE_ID, "scv", [REPORTE_SUPERVISORES_ASIGNADOS_ID, REPORTE_SUPERVISORES_NO_ASIGNADOS_ID]);
      ...
      export const Edicion: any = page.connect(class extends page.Base {
        ...
        onEntitySaved(props: page.IProps): void {
            ...
            let item = getData(EK.Store.getState().global.SPVSupervisoresCoordinadores$plazaSeleccionada);
            ...
            dispatchAsyncPost("global-page-data", "base/scv/SPVSupervisoresCoordinadores/GetBP/getCoordinadores/", { parametros: p }, REPORTE_USUARIOS_ID);
            delete EK.Store.getState().global.catalogoOriginalUPP;
            delete EK.Store.getState().global.catalogoOriginalSA;
            delete EK.Store.getState().global.catalogoOriginalSNA;
        };
    ...
    const listHeaderUsuarios: JSX.Element =
        <Column size={[12, 12, 12, 12]}>
            <Row>
                ...
                <Column size={[6, 6, 6, 6]} className="list-default-header">{"Coordinadores"}</Column>
                <Column size={[3, 3, 3, 3]} className="list-default-header">{"Superv. Configurados"}</Column>
        ...
        onClickItem(item: any) {
            ...
            if (item.ID != null && item.ID != undefined) {
                ...
                let p: any = global.assign({
                    ...
                    IdCoordinador: item.Clave
                });

                dispatchAsyncPost("global-page-data", "base/scv/SPVSupervisoresCoordinadores/GetBP/getAsignado/", { parametros: p }, REPORTE_SUPERVISORES_ASIGNADOS_ID);
                dispatchAsyncPost("global-page-data", "base/scv/SPVSupervisoresCoordinadores/GetBP/getNoAsignado/", { parametros: p }, REPORTE_SUPERVISORES_NO_ASIGNADOS_ID);
                delete EK.Store.getState().global.catalogoOriginalSA;
                delete EK.Store.getState().global.catalogoOriginalSNA;
            }

        }
        // Al pasar los parametros a la funcion, el objeto 'this' debe ir en primera posicion, pero siempre se va a obtener en la ultima posicion ya dentro de la funcion
    ...
    let ViewSPVSupervisorsAsignados: any = global.connect(class extends React.Component<IViewSupervisorsProps, IViewSupervisorsProps> {
        ...
        static props: any = (state: any) => {
            var retValue: any = page.props(state);
            retValue.fraccionamientos = state.global["catalogo$" + REPORTE_SUPERVISORES_ASIGNADOS_ID];
            retValue.fraccionamientosAsignados = Forms.getValue(REPORTE_SUPERVISORES_ASIGNADOS_ID, config.id);
            retValue.fraccionamientosNoAsignados = Forms.getValue(REPORTE_SUPERVISORES_NO_ASIGNADOS_ID, config.id);
            return retValue;
        };
                    ...
                    formatter={(index: number, item: any) => {
                        return <Row style={{ padding: "0px 10px" }}>
                            ...
                            <Column size={[1, 1, 1, 1]} className="listItem-center-header">
                                {editarElemento === true ?
                                    <i className="fas fa-times" title="Eliminar Supervisor" style={{ color: "red", cursor: "pointer" }} onClick={(e) => this.eliminarElemento(item, e)}        ></i>
                                    : null}
                            </Column>
                        </Row>;
                    }}>
    ...
    let ViewSPVSupervisorsNoAsignados: any = global.connect(class extends React.Component<IViewSupervisorsProps, IViewSupervisorsProps> {
        ...
        static props: any = (state: any) => {
            var retValue: any = page.props(state);
            retValue.fraccionamientos = state.global["catalogo$" + REPORTE_SUPERVISORES_NO_ASIGNADOS_ID];
            retValue.fraccionamientosAsignados = Forms.getValue(REPORTE_SUPERVISORES_ASIGNADOS_ID, config.id);
            retValue.fraccionamientosNoAsignados = Forms.getValue(REPORTE_SUPERVISORES_NO_ASIGNADOS_ID, config.id);
            return retValue;
        };
                ...
                <page.SectionList
                    ...
                    formatter={(index: number, item: any) => {
                        return <Row style={{ padding: "0px 10px" }}>
                            ...
                            <Column size={[1, 1, 1, 1]} className="listItem-center-header">
                                {editarElemento === true ?
                                    <i className="fa fa-plus-square" title="Agregar Supervisor" style={{ color: "blue", cursor: "pointer" }} onClick={(e) => this.eliminarElemento(index, item, e)}        ></i>
                                    : null}
                            </Column>
                        </Row>;
                    }}>
