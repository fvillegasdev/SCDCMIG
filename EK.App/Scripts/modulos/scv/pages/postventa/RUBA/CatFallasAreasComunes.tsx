namespace EK.Modules.SCV.Pages.postventa.RUBA.CatFallasAreasComunes {
    "use strict";
    const PAGE_ID = "catFallasAreasComunes";
    const config: page.IPageConfig = global.createPageConfig(PAGE_ID, "scv");

    interface ICatFallasAreasComunes extends page.IProps {
        modeEdit?: any
    };
    ...
    export let Vista = global.connect(class extends React.Component<ICatFallasAreasComunes, {}> {
        constructor(props: ICatFallasAreasComunes) {
            super(props);
        ...
        loadData() {
            const columns = [
                ...
                { caption: "Falla", dataField: "Falla" },
                { caption: "Id Tipo Falla", dataField: "IdTipoFalla", alignment: 'center' },
                { caption: "Tipo Falla", dataField: "TipoFalla" },
                { caption: "Duración", dataField: "Duracion", alignment: 'center' },
                { caption: "Impacto", dataField: "Impacto" },
            ]

            let loader = document.getElementById('loading');
            let loadedTable = document.getElementById('loadedData');

            global.asyncPost("base/kontrol/catFallasAreasComunes/GetBP/GetCatalogo/", { parametros: null }, (status: AsyncActionTypeEnum, data: any) => {
                switch (status) {
                    case AsyncActionTypeEnum.successful:
                        loader.style.display = 'none';
                        loadedTable.style.display = 'inherit';
                        dispatchSuccessful('global-page-data', data, "catFallasAreasComunes");
                        let fecha = Date.now();
                        let dataGrid = $("#datagroupContainer").dxDataGrid({
                            onRowPrepared: function (event) {
                                $(event.rowElement).on('dblclick', function () {
                                    ...
                                    Forms.updateFormElement(PAGE_ID, "TipoFalla", { ID: event.data.IdTipoFalla, Clave: event.data.IdTipoFalla, Nombre: event.data.TipoFalla });
                                    Forms.updateFormElement(PAGE_ID, "Falla", event.data.Falla);
        ...
        onsave() {
            ...
            let Falla = $.trim(model.Falla);
            let Duracion = $.trim(model.Duracion);
            let Impacto = model.Impacto;

            if (Duracion === undefined || Duracion === "" || Falla === undefined || Falla === "" || model.Impacto === null) {
            ...
            global.asyncPost("base/kontrol/catFallasAreasComunes/GetBP/CrudCatalogoFallaAreaComun/", { parametros: parametros }, (status: AsyncActionTypeEnum, data: any) => {
                switch (status) {
                    case AsyncActionTypeEnum.successful:
                        if (data[0].code === 0) {
        ...
        onEdit() {
            ...
            let regexNum = /[0-9]/;
            if ((model.Duracion !== null || model.Duracion !== undefined) && !regexNum.test(model.Duracion)) {
                console.log(regexNum.test(model.Duracion))
                warning("Favor solo introducir numeros en el campo Duracion", "Aviso");
                return;
            }
            ...
            return <page.Main {...config} pageMode={PageMode.Personalizado}>
                ...
                <page.OptionSection
                    ...
                    <Column size={[12, 12, 12, 12]} style={{ padding: '10px' }}>
                        <TipoFallasAreaComunDDL id={"TipoFalla"} size={[12, 12, 4, 4]} label={"Tipo Falla"} idFormSection={PAGE_ID} required={true} validations={[validations.required()]}/> 
                        <input.Text id="Falla" label="Falla" idFormSection={PAGE_ID} size={[12, 12, 4, 4]} required={true} validations={[validations.required()]}/>
                        <input.Text id="Duracion" label="Duracion" idFormSection={PAGE_ID} size={[12, 12, 4, 4]} required={true} validations={[validations.required()]}/>
                    </Column>
                    <Column size={[12, 12, 12, 12]} style={{  }}>
                        ...
                        <RadioButton id={"NORMAL"} label = "NORMAL" idForm={config.id} value="M" groupName="Impacto" size={[4, 4, 4, 4]} />
                    </Column>
