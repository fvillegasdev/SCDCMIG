namespace EK.Modules.SCV.Pages.postventa.RUBA.MesaDirectiva {
    "use strict";
    const PAGE_ID = "MesaDirectiva";
    ...
    interface IMesaDirectiva extends page.IProps {
        modeEdit: any,
    ...
    let columnsIntegrantes = [
        ...
        { caption: "Apellido Materno", dataField: "ApellidoMaterno" },
        ...
        //            Forms.updateFormElement(INTEGRANTES, "NoCliente", data.NoCliente)
        //            Forms.updateFormElement(INTEGRANTES, "ApePaterno", data.ApePaterno)
        //            Forms.updateFormElement(INTEGRANTES, "ApeMaterno", data.ApeMaterno)
    ...
    let getData = () => {
        ...
        const columns = [
            ...
            //{ caption: "Nombre Comite Vecinal", dataField: "NombreComiteVecinal" },
            ...
            { caption: "Fondo Economico Inicial", dataField: "FondoEconomicoInicial" },
            ...
            //{ caption: "Hipoteca con Servicios", dataField: "HipotecaServicios" },
            {
                ...
                buttons: ['edit', {
                    ...
                    onClick(e) {
                        ...
                        Forms.updateFormElement(PAGE_ID, "NombreComiteVecinal", data.NombreComiteVecinal);
                        ...
                        //Forms.updateFormElement(PAGE_ID, "Vocaciones", { ID: data.IdSegmento, Segmento: data.Segmento });
                        ...
                        Forms.updateFormElement(PAGE_ID, "FondoEconomicoInicial", data.FondoEconomicoInicial);
                        ...
                        getIntegrantesByMD(data.IdComite)

                        dispatchSuccessful("load::Integrantes", data.Integrantes, PAGE_ID)
            ...
            },
            {
                ...
                buttons: ['delete', {
                    ...
                    onClick(e) {
                        ...
                        Forms.updateFormElement(PAGE_ID, "NombreComiteVecinalLabel", row.NombreComiteVecinal);
                        ...
                        Forms.updateFormElement(PAGE_ID, "FondoEconomicoInicialLabel", row.FondoEconomicoInicial);
        ...
        global.asyncPost("base/kontrol/MesaDirectiva/GetBP/GetMesaDirectiva/", { parametros: parametros }, (status: AsyncActionTypeEnum, data: any) => {
            switch (status) {
                case AsyncActionTypeEnum.successful:
                    console.log(data)
    ...
    let onDelete = (Id) => {
        ...
        EK.Global.confirmTrueFalse("¿Esta seguro de eliminar el registro?", "Eliminar", () => {
            ...
            global.asyncPost("base/kontrol/MesaDirectiva/GetBP/DeleteInfoMesaDirectiva/", { parametros: parametros }, (status: AsyncActionTypeEnum, data: any) => {
                switch (status) {
                    case AsyncActionTypeEnum.successful:
                        loader.style.display = 'none';
                        loaded.style.display = 'inherit';
                        if (data[0].code === 0) {
                            ...
                            Forms.updateFormElement(PAGE_ID, "FechaContitucion", null)
                            ...
                            Forms.updateFormElement(PAGE_ID, "FondoEconomicoInicial", null)
                            Forms.updateFormElement(PAGE_ID, "CuotaMantenimiento", null)
    ...
    let getIntegrantesByMD = (Id: number) => {
        let parametros = global.assign({
            IDCOMITE: Id
        })
        global.dispatchAsyncPost("load::Integrantes", "base/kontrol/AsociacionCivil/GetBP/GetIntegrantesComite/", { parametros: parametros });
    }
    export let Vista = global.connect(class extends React.Component<IMesaDirectiva, {}> {
        constructor(props: IMesaDirectiva) {
            super(props);
        ...
        componentWillReceiveProps(nextProps: IMesaDirectiva): void {
        ...
        onsave() {
           ...
           // let Nombre = model.NombreComiteVecinal;
            ...
            let FondoEconomicoInicial = model.FondoEconomicoInicial;
            ...
            //    warning("El Campo Nombre o Comite Vecinal es Obligatorio", "Atencion");
            //    return;
            //}

            //if (Plaza === -2) {
            //    warning("La opcion Todos en el campo Plazas es incorrecta, favor de seleccionar otra opcion", "Atencion");
            //    return;
            //}
            //if (Segmento === -2) {
            ...
            if (FondoEconomicoInicial === undefined || FondoEconomicoInicial === null || FondoEconomicoInicial === "") {
                warning("Favor de seleccionar una opcion en el campo Fondo Economico Inicial", "Atencion");
                return;
            }
            ...
            parametros.push({
                ...
                FondoEconomicoInicial: FondoEconomicoInicial,
            ...
            global.asyncPost("base/kontrol/MesaDirectiva/GetBP/Save/", { parametros: parametros }, (status: AsyncActionTypeEnum, data: any) => {
                switch (status) {
                    case AsyncActionTypeEnum.successful:
                        if (data > 0) {
                            getData()
                            ...
                            warning("Ya existe una mesa directiva en el fraccionamiento seleccionado, verifique la información", "Aviso");
                            loader.style.display = 'none';
                            loaded.style.display = 'inherit';
                            Forms.reset(PAGE_ID);
                            Forms.updateFormElement(PAGE_ID, "FraccInicial", { ID: -1, Clave: "Seleccione una opción", Descripcion: "Seleccione una opción" });
    ...
    // CREATE 
    //=========================================================================
    const MesaDirectiva: any = global.connect(class extends React.Component<IMesaDirectiva, {}>{
        constructor(props: IMesaDirectiva) {
            super(props);
        };
        static props: any = (state: any) => {
        ...
        shouldComponentUpdate(nextProps: IMesaDirectiva, { }): boolean {
        ...
        componentWillReceiveProps(nextProps: IMesaDirectiva): void {
            ...
            if (isSuccessful(nextProps.FraccSel) && hasChanged(this.props.FraccSel, nextProps.FraccSel)) {
                console.log(this.props.FraccSel)
                console.log(nextProps.FraccSel)
                if (global.getData(nextProps.FraccSel).ID && global.getData(nextProps.FraccSel).ID !== -1  && global.getData(this.props.FraccSel).ID !== global.getData(nextProps.FraccSel).ID) {
            ...
            //if (isSuccessful(nextProps.FraccSel) && hasChanged(this.props.FraccSel, nextProps.FraccSel)) {
            //    if (global.getData(nextProps.FraccSel).ID > 0 && global.getData(this.props.FraccSel).ID !== global.getData(nextProps.FraccSel).ID) {
            //        let data: any = global.getData(nextProps.FraccSel);
            //        let Id = data.ID;
            //        dispatchSuccessful("load::loadComite", { loadComite: true })
            //        let modeEdit = EK.Store.getState().global.modeEdit.data.modeEdit
            ...
            //                case AsyncActionTypeEnum.successful:
            //                    if (res) {
            //                        dispatchSuccessful('load::Comite', res)
            //                        Forms.updateFormElement(PAGE_ID, "PlazaLabel", res.Plaza)
            //                        Forms.updateFormElement(PAGE_ID, "VocacionesLabel", res.Segmento)
            //                        Forms.updateFormElement(PAGE_ID, "Comite", res.Nombre)
            //                        setTimeout(() => {
            ...
            //                        dispatchSuccessful("load::loadComite", { loadComite: false })
            //                        if (res === null) {
            //                            warning("El fraccionamiento no cuenta con un comite asignado", "Atención")
            //                        }
            //                    }
            //                    break;
            //                case AsyncActionTypeEnum.loading:
                                ...
                                <Column size={[12, 12, 12, 12]}>
                                    <page.OptionSection
                                        ...
                                        <Row>
                                            ...
                                            <Column size={[12, 12, 12, 12]}>
                                                <Column size={[12, 12, 12, 6]}>
                                                    <input.Currency id={"FondoEconomicoInicial"} label={"Fondo Economico Inicial"} size={[12, 12, 12, 12]} idForm={PAGE_ID} validations={[validations.required()]} required={true} />
                                                </Column>
                                                <Column size={[12, 12, 12, 6]}>
                                            ...
                                            {/*<Column size={[12, 12, 12, 4]}>
                                                <Column size={[12, 12, 12, 12]}>
                                                    <span><strong>HIPOTECA CON SERVICIOS</strong></span>
                                                </Column>
                                                <Column size={[12, 12, 12, 12]}>
                                                    <RadioButton id="SIH" label="SI" idForm={PAGE_ID} groupName="Hipoteca" size={[3, 3, 6, 6]} />
                                                    <RadioButton id="NOH" label="NO" idForm={PAGE_ID} groupName="Hipoteca" size={[3, 3, 6, 6]} />
                                                </Column>
                                            </Column>*/}
                                        </Row>
                                    </page.OptionSection>
    ...
    const ModalIntegrantes: any = global.connect(class extends React.Component<IMesaDirectiva, {}>{
        constructor(props: IMesaDirectiva) {
            super(props);
        };
        static props: any = (state: any) => {
    ...
    const ModalDetalle: any = global.connect(class extends React.Component<IMesaDirectiva, {}>{
        constructor(props: IMesaDirectiva) {
            super(props);
        };
        static props: any = (state: any) => {
        ...
        onExport() {
            ...
            let mesaDHeaders = ['Id', 'Comite','Fecha Constitución', 'Administradora', 'Plaza', 'Segmento',
                'Fraccionamiento',
                'Fondo Economico Inicial',
            ...
            let mesaDKeys = ['ID', 'NombreComite', 'FechaConstitucion', 'Administradora', 'Plaza', 'Segmento', 'Fraccionamiento',
                'FondoEconomicoInicial',
                'PorcentajeEntregado',
                'CuotaMantenimiento',
                'FondoConvive',
               // 'HipotecaServicios'
            ];
            let mesaDRow = worksheet.addRow(mesaDHeaders);
            mesaDRow.font = { bold: true };
            mesaDRow.font = headerStyle.font;
            mesaDRow.fill = headerStyle.fill;
            mesaDRow.eachCell(cell => cell.border = borderStyle);

            let dataRow = [];
            for (let key of mesaDKeys) {
            ...
            workbook.xlsx.writeBuffer().then(function (buffer) {
                saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'MesaDirectivaDetalles.xlsx');
            });
            ...
            return <modal.Modal id="ModalDetalles" header={"DETALLE"} footer={this.footerModal()}
                ...
                <Row style={{ marginLeft: "15px", marginRight: "15px" }}>
                    ...
                    <Column size={[12, 12, 12, 12]}>
                        ...
                        <label.Label id={"FondoEconomicoInicialLabel"} size={[12, 12, 12, 4]} label={"Fondo Economico Inicial"} idForm={PAGE_ID} />
