namespace EK.Modules.SCV.Pages.Clientes {
    ...
    export const Edicion: any = global.connect(class extends React.Component<IPageEditProps, IPageEditProps> {
        ...
        onEntityLoaded(props: page.IProps): any {
            ...
            if (idCliente <= 0 || idCliente === undefined) {
                ...
                props.config.dispatchCatalogoBase("base/kontrol/ScvClientes/Get/GetContactoClientes/", parametros, CORREOS_ID);
                parametros = global.assign({ idCliente, ClaveTipoContacto:"TELEFONO" });
                props.config.dispatchCatalogoBase("base/kontrol/ScvClientes/Get/GetContactoClientes/", parametros, TELEFONOS_ID);
                parametros = global.assign({ idCliente});
                props.config.dispatchCatalogoBase("base/kontrol/ScvClientes/Get/GetAsesoresClientes/", parametros, ASESORES);
                props.config.dispatchCatalogoBase("base/kontrol/ScvClientes/Get/GetReferencias/", parametros, REF_PERSONALES_ID);
                props.config.dispatchCatalogoBase("base/kontrol/ScvClientes/Get/GetReferenciasLaborales/", parametros, REF_LABORALES_ID);

                props.config.dispatchCatalogoBase("base/scv/scvclientes/Get/GetBoletasProspeccionPorCliente/", parametros, BOLETAS);
                //
                setGeoInfo(cliente);
        ...
        onWillEntityLoad(id: any, props: page.IProps): void {
            ...
            global.dispatchAsync("global-current-entity", "base/kontrol/ScvClientes/Get/GetByClienteId/"+ parametros);
        };
        getAddress(): string {
    ...
    let ExpMapViewerButton: any = global.connect(class extends React.Component<IMapViewerButtonProps, {}> {
        ...
        onClick(e: any): any {
            ...
            //
            if (isReadMode === true) {
                cliente = global.getData(this.props.entidad);
            }
            else {
                cliente = item
                    .addString("Geolocalizacion")
            ...
            if (isReadMode === true && !cliente.Geolocalizacion) {
                global.warning("No se ha establecido la información de geo-localización");
                return;
            };

            setGeoInfo(cliente, isReadMode);
            //
    ...
    export let CientesSinExpedientes: any = global.connect(class extends React.Component<IClientesSinExpediente, IClientesSinExpediente> {
        constructor(props: IClientesSinExpediente) {
            super(props);
        ...
        static props: any = (state: any) => {
            return {
                items : state.global.catalogo$ClientesSinExpediente
            };
        };
        onCollapse(collapsed: boolean): any {
            if (collapsed === true && !global.isLoadingOrSuccessful(this.props.items)) {
                let parametros: any = { parametros: { operacion: "Cliente" } };

                global.dispatchAsync("global-page-data", "base/kontrol/ScvClientes/Get/GetClientWithoutFile/" + global.encodeObject(parametros), "ClientesSinExpediente");
            };
        };
        componentDidMount() {
        ...
        render(): JSX.Element {
            ...
            dtConfig.columns
                .addLinkCliente({ data: "NombreCompleto", flex: { grow: 1 } })
                .addDateFormat({ data: "FechaContacto", width: "100px" })
                .addNumber({ title: "Días", data: "DiasTranscurridos", width: "50px" });
            //sidebarContent={<h1>contenido</h1>}
            //onRowSelected={() => { global.showSidebar("ClientesSinExpediente$sb"); }}
    ...
    let SectionBoletasFieldsRendering: any = global.connect(class extends React.Component<ISectionBoletasFieldsRendering, ISectionBoletasFieldsRendering> {
        ...
        render(): JSX.Element {
            ...
            if (formTipoFinanciamiento != null && formTipoFinanciamiento != undefined && formTipoFinanciamiento.ID && formTipoFinanciamiento.ID > 0) {
                ...
                tipoEntidad = "boletasProspeccion";
            }
